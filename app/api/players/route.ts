...
function sortDemo(category:string,chain:string){
 ...
 return DEMO.filter(p=>chain==="all"||p.chain===chain).sort((a:any,b:any)=>b[key]-a[key]).map((p,i)=>({...p,rank:i+1}));
}
...
export async function GET(request:NextRequest){
 ...
 if(!dbEnabled()) return NextResponse.json({season:1,category,chain,players:sortDemo(category,chain).slice(0,limit),source:"demo"});
