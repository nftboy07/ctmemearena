...
export async function GET(request:NextRequest){
 ...
 for(const u of users.rows){try{const r=await fetch(rpc(),{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({jsonrpc:"2.0",id:1,method:"getSignaturesForAddress",params:[u.wallet,{limit:20}]})});const j=await r.json();if(!r.ok||j.error)continue;for(const x of j.result??[]){const q=await query(`INSERT INTO arena_events(wallet,event_type,payload) SELECT $1,'WALLET_ACTIVITY',$2 WHERE NOT EXISTS (SELECT 1 FROM arena_events WHERE event_type='WALLET_ACTIVITY' AND payload->>'signature'=$3)`,[u.wallet,JSON.stringify(x),x.signature]);synced+=q.rowCount??0;}}catch{}}
