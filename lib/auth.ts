...
export async function currentPrivyUser() {
  ...
  try {
    return await client().utils().auth().verifyAuthToken(token);
  } catch {
    return null;
  }
}
...
export async function currentUser() {
  ...
  const accounts = user.linked_accounts as any[];
  const wallet = accounts.find((account) => account.type === "wallet" && account.chain_type === "solana");
  ...
  const apple = accounts.find((account) => account.type === "apple_oauth");
  ...
  return {
    ...
    google: google?.subject ?? null,
    twitter: twitter?.subject ?? null,
    twitterUsername: twitter?.username ?? null,
    apple: apple?.subject ?? null,
    avatarUrl: twitter?.profile_picture_url ?? twitter?.profilePictureUrl ?? null,
  };
}
...
export async function currentWallet() {
  ...
  if (current?.wallet) {
    if (dbEnabled()) {
      await query(
        `INSERT INTO users (privy_user_id,wallet,email,google_subject,twitter_subject,apple_subject,twitter_username,avatar_url,display_name)
         VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)
         ON CONFLICT (privy_user_id) DO UPDATE SET
           wallet=EXCLUDED.wallet,
           email=EXCLUDED.email,
           google_subject=EXCLUDED.google_subject,
           twitter_subject=EXCLUDED.twitter_subject,
           apple_subject=EXCLUDED.apple_subject,
           twitter_username=COALESCE(EXCLUDED.twitter_username,users.twitter_username),
        ...
        [
          ...
          current.twitter,
          current.apple,
          current.twitterUsername,
          current.avatarUrl,
          current.twitterUsername || current.email?.split("@")[0] || "Player",
        ],
      );
      await query(`INSERT INTO player_stats(wallet) VALUES($1) ON CONFLICT(wallet) DO NOTHING`, [current.wallet]);
    }
  ...
  const result = await query<{ wallet: string }>(
    `SELECT wallet FROM wallet_sessions WHERE token_hash=$1 AND revoked_at IS NULL AND expires_at>NOW()`,
    [hash(session)],
  );
...
export async function requireWallet() {
  ...
  if (!wallet) throw new Error("Authenticated Privy Solana wallet required");
  return wallet;
}

export async function requirePrivyUser() {
  const user = await currentUser();
  if (!user) throw new Error("Privy authentication required");
  return user;
}

export async function logout() {
  const current = await currentUser();
  if (current?.wallet && dbEnabled()) {
    await query(`UPDATE wallet_sessions SET revoked_at=NOW() WHERE wallet=$1 AND revoked_at IS NULL`, [current.wallet]);
  }
